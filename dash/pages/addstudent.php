<?php
		...
		if(isset($_POST['sub'])){
			...
			//$mec = $_POST['mec'];
			$red = mysqli_query($connect,"select * from Teachers where Branch = '$branch' and Name ='$atc'");
			$red = mysqli_fetch_assoc($red);
			$atc = $red['Email'];
			$red = mysqli_query($connect,"select * from Teachers where Branch = '$branch' and Name ='$guide'");
			$red = mysqli_fetch_assoc($red);
			$guide = $red['Email'];
			$red = mysqli_query($connect,"select * from Teachers where Branch = '$branch' and Name ='$faculty'");
			$red = mysqli_fetch_assoc($red);
			$faculty = $red['Email'];
			//$red = mysqli_query($connect,"select * from Teachers where Branch = '$branch' and Name ='$mec'");
			//$red = mysqli_fetch_assoc($red);
			$mec = $email;
			$response = mysqli_query($connect,"Insert into Students values('$roll','$name','$email1','$ctc','$branch','$batch','$atc','$guide','$faculty','$mec')");
		}
?>
<html lang="en">
<head>
            ...
            <div class="row">
                 <div class="col-lg-12">
                    <div class="panel panel-default">
						...
						<form method = "post">
						<input class="form-control" placeholder="Roll Number" name ="roll">
						...
						<input class="form-control" placeholder="Year of Admission" name ="batch">
						<label>Guide</label>
						<select class="form-control" name = "guide">
						<option>NA</option>
                        <?php
						$red = mysqli_query($connect,"select * from Teachers where Branch = '$branch'");
						while ($re = mysqli_fetch_array($red)){
							$nm = $re['Name'];
							echo "<option>$nm</option>";
						}
						?>
                        </select>
						<label>TA Faculty</label>
